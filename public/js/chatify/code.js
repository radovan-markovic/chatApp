...
function fetchMessages(id, type) {
    if (messenger != 0) {
        $.ajax({
            ...
            method: 'POST',
            data: { '_token': access_token, 'id': 0, 'type': 'user' },
            dataType: 'JSON',
            success: (data) => {
...
fetchMessages(0, 'user');
scrollBottom(messagesContainer);
messageInput.css({ 'height': '42px' });
// form reset and focus
$('.messenger-sendCard').css('display','block');
messageInput.removeAttr('readonly');
$('#message-form button').removeAttr('disabled');
$('.upload-attachment').removeAttr('disabled');
$("#message-form").trigger("reset");
...
$(document).ready(function () {
    ...
    var message = $('#sendMessageToOthers').val();
    $('#sendMessageToOthers').val(urlify(message));
console.log(urlify(message))
    ...
    //add emotions instead of smilies
    var map = {
        "<3": "\u2764\uFE0F",
        "</3": "\uD83D\uDC94",
        ":D": "\uD83D\uDE00",
    ...
    document.getElementById('sendMessageToOthers').oninput = function() {
        for (var i in map) {
